...
UI = collections.OrderedDict([
    ...
    ("Diagnose", {"en": "Diagnostics", "fr": "Diagnostic", "es": "Diagnóstico", "it": "Diagnostica", "pl": "Diagnostyka"}),
    ("Ausführliches Protokoll", {"en": "Verbose log", "fr": "Journal détaillé", "es": "Registro detallado", "it": "Log dettagliato", "pl": "Szczegółowy dziennik"}),
    ("Protokoll-Ordner öffnen", {"en": "Open log folder", "fr": "Ouvrir le dossier des journaux", "es": "Abrir carpeta de registros", "it": "Apri cartella dei log", "pl": "Otwórz folder dziennika"}),
    ("Schreibt zusätzlich regelmäßig Speicher- und Thread-Werte ins Protokoll — nützlich, um über mehrere Tage Lecks aufzuspüren. Aus reicht das normale Protokoll mit Fehlern und Zeiten.",
     {"en": "Additionally records memory and thread values to the log at intervals — useful for tracking down leaks over several days. When off, the normal log with errors and timings is enough.",
      "fr": "Enregistre en plus régulièrement la mémoire et les threads dans le journal — utile pour traquer des fuites sur plusieurs jours. Désactivé, le journal normal avec erreurs et temps suffit.",
      "es": "Además registra periódicamente valores de memoria e hilos en el registro — útil para rastrear fugas durante varios días. Desactivado, basta el registro normal con errores y tiempos.",
      "it": "Registra inoltre a intervalli i valori di memoria e thread nel log — utile per individuare perdite nell'arco di più giorni. Disattivato, basta il log normale con errori e tempi.",
      "pl": "Dodatkowo regularnie zapisuje wartości pamięci i wątków do dziennika — przydatne do wykrywania wycieków w ciągu kilku dni. Wyłączony — wystarcza zwykły dziennik z błędami i czasami."}),
    ("Das Protokoll liegt unter ~/Library/Logs/Spitr und enthält nie deinen diktierten Text — nur Ereignisse, Zeiten und Fehler. Es bleibt komplett auf deinem Mac.",
     {"en": "The log lives in ~/Library/Logs/Spitr and never contains your dictated text — only events, timings and errors. It stays entirely on your Mac.",
      "fr": "Le journal se trouve dans ~/Library/Logs/Spitr et ne contient jamais ton texte dicté — uniquement des événements, des temps et des erreurs. Il reste entièrement sur ton Mac.",
      "es": "El registro se encuentra en ~/Library/Logs/Spitr y nunca contiene tu texto dictado — solo eventos, tiempos y errores. Permanece por completo en tu Mac.",
      "it": "Il log si trova in ~/Library/Logs/Spitr e non contiene mai il testo dettato — solo eventi, tempi ed errori. Resta interamente sul tuo Mac.",
      "pl": "Dziennik znajduje się w ~/Library/Logs/Spitr i nigdy nie zawiera dyktowanego tekstu — tylko zdarzenia, czasy i błędy. Pozostaje w całości na Twoim Macu."}),
    ...
    ("Sprachisolierung (experimentell)", {"en": "Voice isolation (experimental)", "fr": "Isolation de la voix (expérimental)", "es": "Aislamiento de voz (experimental)", "it": "Isolamento vocale (sperimentale)", "pl": "Izolacja głosu (eksperymentalna)"}),
    ("Aufgenommen wird nur, solange du die Taste hältst. Die Sprachisolierung kann stetige Hintergrundgeräusche (z. B. Fernseher, Lüfter) filtern. Sie nutzt Apples Telefonie-Tonverarbeitung, die nicht auf jedem Mac startet — klappt sie nicht, schaltet Spitr automatisch auf das normale Mikrofon zurück. Standardmäßig aus; bei viel Umgebungslärm einen Versuch wert.",
     {"en": "Recording happens only while you hold the key. Voice isolation can filter steady background noise (e.g. a TV or fan). It uses Apple's VoIP audio processing, which doesn't start on every Mac — if it fails, Spitr automatically falls back to the plain microphone. Off by default; worth a try with a lot of ambient noise.",
      "fr": "L'enregistrement n'a lieu que tant que tu maintiens la touche. L'isolation de la voix peut filtrer les bruits de fond constants (p. ex. une télé ou un ventilateur). Elle utilise le traitement audio VoIP d'Apple, qui ne démarre pas sur tous les Mac — si elle échoue, Spitr revient automatiquement au microphone normal. Désactivée par défaut ; à essayer en cas de bruit ambiant important.",
      "es": "La grabación solo ocurre mientras mantienes la tecla. El aislamiento de voz puede filtrar el ruido de fondo constante (p. ej. un televisor o un ventilador). Usa el procesamiento de audio VoIP de Apple, que no se inicia en todos los Mac — si falla, Spitr vuelve automáticamente al micrófono normal. Desactivado por defecto; vale la pena probarlo con mucho ruido ambiental.",
      "it": "La registrazione avviene solo mentre tieni premuto il tasto. L'isolamento vocale può filtrare i rumori di fondo costanti (es. una TV o una ventola). Usa l'elaborazione audio VoIP di Apple, che non si avvia su tutti i Mac — se non funziona, Spitr torna automaticamente al microfono normale. Disattivato per impostazione predefinita; vale un tentativo con molto rumore ambientale.",
      "pl": "Nagrywanie odbywa się tylko, gdy trzymasz klawisz. Izolacja głosu może filtrować stały szum tła (np. telewizor lub wentylator). Wykorzystuje przetwarzanie dźwięku VoIP firmy Apple, które nie uruchamia się na każdym Macu — jeśli zawiedzie, Spitr automatycznie wraca do zwykłego mikrofonu. Domyślnie wyłączona; warto spróbować przy dużym hałasie otoczenia."}),
    ...
    ("Doppelton", {"en": "Double beep", "fr": "Double bip", "es": "Doble pitido", "it": "Doppio bip", "pl": "Podwójny sygnał"}),
    ("Aufsteigend (Funk)", {"en": "Rising (radio)", "fr": "Ascendant (radio)", "es": "Ascendente (radio)", "it": "Ascendente (radio)", "pl": "Rosnący (radio)"}),
    ...
    ("Kurze Töne, wenn das Mikro wirklich aufnimmt (Beginn) und wenn der Text eingefügt wurde (Ende) — so verlierst du das erste Wort nicht und weißt, wann die Umwandlung fertig ist. Den Stil hörst du beim Umschalten gleich probe.",
     {"en": "Short tones when the mic actually starts recording (beginning) and when the text has been inserted (end) — so you don't lose the first word and know when the conversion is done. You hear the style as you switch it.",
      "fr": "De brefs sons quand le micro commence vraiment à enregistrer (début) et quand le texte a été inséré (fin) — pour ne pas perdre le premier mot et savoir quand la conversion est terminée. Vous entendez le style en le changeant.",
      "es": "Tonos breves cuando el micrófono empieza a grabar (inicio) y cuando el texto se ha insertado (fin) — así no pierdes la primera palabra y sabes cuándo termina la conversión. Oyes el estilo al cambiarlo.",
      "it": "Brevi segnali acustici quando il microfono inizia davvero a registrare (inizio) e quando il testo è stato inserito (fine) — così non perdi la prima parola e sai quando la conversione è finita. Senti lo stile mentre lo cambi.",
      "pl": "Krótkie dźwięki, gdy mikrofon faktycznie zaczyna nagrywać (początek) i gdy tekst został wstawiony (koniec) — dzięki temu nie tracisz pierwszego słowa i wiesz, kiedy konwersja się kończy. Styl słyszysz przy jego przełączaniu."}),
